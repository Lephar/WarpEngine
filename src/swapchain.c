...
void createSwapchain() {
    ...
    for(uint32_t imageIndex = 0; imageIndex < swapchain.imageCount; imageIndex++) {
        swapchain.images[imageIndex] = wrapImage(handles[imageIndex], extent.width, extent.height, 1, VK_SAMPLE_COUNT_1_BIT, surfaceFormat.format, VK_IMAGE_USAGE_TRANSFER_DST_BIT, VK_IMAGE_ASPECT_COLOR_BIT);
